...
var YAWN = /*#__PURE__*/function () {
  ...
  _proto.getRemark = function getRemark(path) {
    var ast = yamlJs.compose(this.yaml);
  ...
  return _createClass(YAWN, [{
    key: "json",
    get: function get() {
      return jsYaml.load(this.yaml);
    },
    set: function set(newJson) {
      // if json is not changed do nothing
      if (lodash.isEqual(this.json, newJson)) {
        return;
      }
      var ast = yamlJs.compose(this.yaml);
      if (lodash.isUndefined(newJson)) {
        this.yaml = '';
        return;
      }
      // -------------------------------------------------------------------------
      ...
      // -------------------------------------------------------------------------
      if (lodash.includes([NULL_TAG, STR_TAG, INT_TAG, FLOAT_TAG], ast.tag)) {
...
function getTag(json) {
  ...
  if (lodash.isArray(json)) {
    tag = SEQ_TAG;
  } else if (lodash.isObject(json)) {
    tag = MAP_TAG;
  } else if (lodash.isNull(json)) {
    tag = NULL_TAG;
  } else if (lodash.isNumber(json)) {
    if (json % 10 === 0) {
      tag = INT_TAG;
    } else {
      tag = FLOAT_TAG;
    }
  } else if (lodash.isString(json)) {
    tag = STR_TAG;
  } else {
...
function updateSeq(ast, newJson, yaml, offset) {
  var values = jsYaml.load(yamlJs.serialize(ast));
...
function updateMap(ast, newJson, json, yaml, offset) {
  // look for changes
  lodash.each(ast.value, function (pair) {
    var keyNode = pair[0],
      valNode = pair[1];
    // node is deleted
    if (lodash.isUndefined(newJson[keyNode.value])) {
    ...
    if (newValue !== value && !lodash.isArray(valNode.value)) {
    ...
    if (!lodash.isEqual(newValue, value) && lodash.isArray(valNode.value)) {
  ...
  lodash.each(newJson, function (value, key) {
    // item is new
    if (lodash.isUndefined(json[key])) {
      var _cleanDump;
...
function getNodeEndMark(ast) {
  if (lodash.isArray(ast.value) && ast.value.length) {
    var lastItem = lodash.last(ast.value);
    if (lodash.isArray(lastItem) && lastItem.length) {
      return getNodeEndMark(lodash.last(lastItem));
    }
...
function indent(str, depth) {
  return str.split(EOL).filter(function (line) {
    ...
    return lodash.repeat(SPACE, depth) + line;
  }).join(EOL);
}
/*
...
function cleanDump(value) {
  var yaml = jsYaml.dump(value).replace(/\n$/, '');
  return yaml;
}
/*
...
exports.YAWNError = YAWNError;
exports.default = YAWN;
//# sourceMappingURL=yawn-yaml.cjs.development.js.map
